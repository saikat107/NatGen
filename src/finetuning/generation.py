...
from src.evaluator.CodeBLEU import calc_code_bleu
from src.evaluator.bleu import _bleu
from src.finetuning.configs import add_args, set_seed, set_dist
from src.finetuning.models import build_or_load_gen_model
from src.finetuning.utils import get_filenames, get_elapse_time, load_and_cache_gen_data, Example
...
def eval_bleu_per_example(
    ...
    for batch in tqdm(eval_dataloader, total=len(eval_dataloader), desc="Eval bleu for {} set".format(split_tag)):
        ...
        with torch.no_grad():
            preds = model.generate(
                ...
                num_beams=args.beam_size, max_length=args.max_target_length
            )
    ...
    with open(output_fn, 'w') as f, open(gold_fn, 'w') as f1, open(src_fn, 'w') as f2:
        for pred_nl, gold in zip(pred_nls, eval_examples):
            ...
            example_result = calc_code_bleu.evaluate_per_example(
                reference=gold.target.strip(), hypothesis=pred_nl.strip(), lang=metadata['lang']
            )
...
def add_language_to_args(args):
    ...
    #          'multi_task', 'refine-concrete', 'refine-commit', 'translate-500']
...
def main():
            ...
            if args.do_eval:
                ...
                if args.do_eval_bleu:
                    ...
                    if dev_bleu_em > best_bleu_em:
                        ...
                        fa.write(
                            "[%d] Best bleu+em (%.2f) does not drop changed for %d epochs, cur bleu+em: %.2f (bleu: "
                            "%.2f, em: %.2f)\n" % (
    ...
    if args.do_eval or args.do_eval:
        ...
        for part in data_part:
            ...
            logger.info("  Batch size = %d", args.eval_batch_size)
            eval_examples, eval_data = load_and_cache_gen_data(
                args, args.test_filename, pool, tokenizer, part, only_src=True, is_sample=False
            )
